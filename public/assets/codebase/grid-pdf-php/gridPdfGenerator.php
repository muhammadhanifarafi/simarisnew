...
class gridPdfGenerator {
	...
	private function printGridPage($startRow, $pageNumber) {
		...
		$originalcolumns = is_array($this->columns) ? count($this->columns) : 0;
		$originalfooterColumns = is_array($this->footerColumns) ? count($this->footerColumns) : 0;
		// calculates rows number on current page
		$numRows = floor(($pageHeight - $this->headerTotal*$originalcolumns - $this->footerTotal*$originalfooterColumns)/$this->rowHeight);
