...
class gridPdfWrapper {
	...
	private function footerDrawAfterRows() {
		...
		$yPos = $this->cb->getPageHeight() - $this->offsetBottom - $this->footerTotal*$originalfooterColumns;
...
//							$this->cb->Cell($width, $height, $this->footerColumns[$i][$j]['text'], 1, 0, 'C', 1);
	...
	public function gridDraw($rowHeight, $rows, $widths, $startRow, $rowsNum, $scaleOneColor, $scaleTwoColor, $profile) {
		...
		$last = '0';
		$originalfooterColumns = is_array($this->footerColumns) ? count($this->footerColumns) : 0;
		$limitY = $this->cb->getPageHeight() - $this->offsetBottom - $originalfooterColumns*$this->footerTotal;
