...
class MasterProfilMitigasiController extends BaseController
{
    protected $MasterProfilMitigasiModel;
    ...
    public function index()
    {
        $data = array_merge($this->data, [
            ...
            'penyebab'           => $this->MasterProfilPenyebabModel->findAll(),
            'mitigasi'           => $this->MasterProfilMitigasiModel->findAll(),
		]);

        return view('master/kamus_risiko/master_profil_mitigasi', $data); // Menampilkan view dengan DataTable dan modal
    }
    ...
    public function getData()
    {
        ...
        $data = $this->MasterProfilMitigasiModel->select('master_profil_mitigasi.id, master_profil_mitigasi.nama_mitigasi, master_profil_mitigasi.ket, master_profil_mitigasi.id_penyebab, master_profil_penyebab.nama_penyebab, master_profil_penyebab.id_peristiwa, master_profil_peristiwa.nama_peristiwa, master_profil_penyebab.id_sasaran, master_profil_sasaran.nama_sasaran')
            ->join('master_profil_penyebab', 'master_profil_mitigasi.id_penyebab = master_profil_penyebab.id')
            ->join('master_profil_peristiwa', 'master_profil_mitigasi.id_peristiwa = master_profil_peristiwa.id')
            ->join('master_profil_sasaran', 'master_profil_mitigasi.id_sasaran = master_profil_sasaran.id')
            ->groupStart()
                ->like('master_profil_mitigasi.nama_mitigasi', $search)
                ->orLike('master_profil_penyebab.nama_penyebab', $search)
                ->orLike('master_profil_peristiwa.nama_peristiwa', $search)
                ->orLike('master_profil_sasaran.nama_sasaran', $search)
            ->groupEnd()
            ->orderBy('master_profil_mitigasi.id', 'desc')
            ->findAll($length, $start);
        
        // Hitung total records tanpa filter
        $totalRecords = $this->MasterProfilMitigasiModel->selectCount('master_profil_mitigasi.id')
            ->join('master_profil_penyebab', 'master_profil_mitigasi.id_penyebab = master_profil_penyebab.id')
            ->join('master_profil_peristiwa', 'master_profil_mitigasi.id_peristiwa = master_profil_peristiwa.id')
            ->join('master_profil_sasaran', 'master_profil_mitigasi.id_sasaran = master_profil_sasaran.id')
            ->countAllResults();
        ...
        $totalFiltered = $this->MasterProfilMitigasiModel->selectCount('master_profil_mitigasi.id')
            ->join('master_profil_penyebab', 'master_profil_mitigasi.id_penyebab = master_profil_penyebab.id')
            ->join('master_profil_peristiwa', 'master_profil_mitigasi.id_peristiwa = master_profil_peristiwa.id')
            ->join('master_profil_sasaran', 'master_profil_mitigasi.id_sasaran = master_profil_sasaran.id')
            ->groupStart()
        ...
        foreach ($data as $item) {
            $result[] = [
                'id' => $item['id'],
                'nama_mitigasi' => $item['nama_mitigasi'],
                'ket' => $item['ket'],
    ...
    public function saveData()
    {
        ...
        $data = [
            'nama_mitigasi' => $this->request->getPost('nama_mitigasi'),
            ...
            'id_penyebab'  => $this->request->getPost('id_penyebab'),
        ];

        if ($id) {
            // Update data jika ID ada
            $this->MasterProfilMitigasiModel->update($id, $data);
            return $this->response->setJSON(['status' => 'success']);
        }
    
        // Insert data baru jika ID tidak ada
        if ($this->MasterProfilMitigasiModel->save($data)) {
            return $this->response->setJSON(['status' => 'success']);
        } else {
    ...
    public function editData($id)
    {
        ...
        $data = $this->MasterProfilMitigasiModel->find($id);
    ...
    public function updateData()
    {
        ...
        if ($this->MasterProfilMitigasiModel->update($id, $data)) {
            return $this->response->setJSON(['status' => 'success']);
        } else {
            return $this->response->setJSON(['status' => 'error']);
        }
    ...
    {
        if ($this->MasterProfilMitigasiModel->delete($id)) {
            return $this->response->setJSON(['status' => 'success']);
        } else {
            return $this->response->setJSON(['status' => 'error']);
        }
